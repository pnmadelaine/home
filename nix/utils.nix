{sources ? import ../sources.nix}: let
  ...
  urlFromLockedInfo = info:
    ...
    then "https://github.com/${info.owner}/${info.repo}/archive/${info.rev}.tar.gz"
    else if info.type == "gitlab"
    then "https://gitlab.com/${info.owner}/${info.repo}/-/archive/${info.rev}.tar.gz"
    else throw "urlFromLockedInfo: node type not recognized";
