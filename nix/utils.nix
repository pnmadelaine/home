...
let
  ...
  getLockedInfo =
    ...
    in
    if lib.lists.isList x then getLockedInfo lock (lib.lists.last x) else lock.nodes.${x}.locked;
  ...
  registryFrom =
    ...
    builtins.mapAttrs (name: value: {
      ...
      to = {
        inherit (value) type;
      } // (if value ? url then { inherit (value) url; } else { inherit (value) owner repo rev; });
      exact = true;
  ...
  mkVarModule = name: value: {
    options.${name} = lib.mkOption { type = lib.types.uniq lib.types.anything; };
    config = {
      ${name} = value;
    };
  };
